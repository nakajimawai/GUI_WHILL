...
class MyApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):  
       ...
       # 介助者操縦モードのシンボル
        ...
        self.img_forward = self.img_forward.resize((int(150 * W), int(150 * H)))
        ...
        self.img_back = self.img_back.resize((int(150 * W), int(150 * H)))
        ...
        self.img_stop = self.img_stop.resize((int(150 * W), int(150 * H)))
        ...
        self.img_lock_forward = self.img_lock_forward.resize((int(150 * W), int(150 * H)))
        ...
        self.img_lock_back = self.img_lock_back.resize((int(150 * W), int(150 * H)))
        ...
        self.cvs_stop_forward.tag_bind(
            self.id_S_F_forward,
            '<Button-1>',
            lambda e: [self.changePage(self.forward_frame), self.change_frame_flag("F"), self.forward(), self.start_blinking(self.id_F_forward)]
            #lambda e: [self.change_frame_flag("F"), self.forward(), self.start_blinking(self.id_forward), self.changePage(self.forward_frame)]
            )

        # 停止シンボル
        self.id_S_F_stop = self.cvs_stop_forward.create_image(
            957,
            333,
        ...
        self.cvs_stop_forward.tag_bind(
            self.id_S_F_stop,
            '<Button-1>',
            lambda e: [self.start_blinking(self.id_S_F_stop)]
            )
        ''''''        
        '''ロボット操作以外のシンボル'''
        # 後方画面に遷移するシンボル
        ...
        self.cvs_stop_back.tag_bind(
            self.id_S_B_back,
            '<Button-1>',
            lambda e: [self.changePage(self.back_frame), self.change_frame_flag("B"), self.back(), self.start_blinking(self.id_B_back)]
            )

        # 停止シンボル
        self.id_S_B_stop = self.cvs_stop_back.create_image(
            957,
            333,
            image = self.img_stop_tk,
            anchor = tk.CENTER,
        ...
        self.cvs_stop_back.tag_bind(
            self.id_S_B_stop,
            '<Button-1>',
            lambda e: [self.start_blinking(self.id_S_B_stop)]
            )
        ''''''
        '''ロボット操作以外のシンボル'''
        # 前方画面に遷移するシンボル
        ...
        '''ロボット操縦シンボル'''
        # 前進シンボル
        self.id_F_forward = self.cvs_forward.create_image(
            337,
            333,
            image = self.img_forward_tk,
            anchor = tk.CENTER,
        ...
        self.cvs_forward.tag_bind(
            self.id_F_forward,
            '<Button-1>',
            lambda e: [self.start_blinking(self.id_F_forward)]
            #lambda e: [self.change_frame_flag("F"), self.forward(), self.start_blinking(self.id_forward), self.changePage(self.forward_frame)]
            )
        ...
        self.cvs_forward.tag_bind(
            self.id_F_stop,
            '<Button-1>',
            lambda e: [self.change_frame_flag("S_F"), self.stop(), self.changePage(self.stop_forward_frame), self.start_blinking(self.id_S_F_stop)]
            )
        ...
        '''ロボット操縦シンボル'''
        # 後退シンボル
        self.id_B_back = self.cvs_back.create_image(
            337,
            333,
            image = self.img_back_tk,
            anchor = tk.CENTER,
        ...
        self.cvs_back.tag_bind(
            self.id_B_back,
            '<Button-1>',
            lambda e: [self.start_blinking(self.id_B_back)]
            )
        ...
        self.cvs_back.tag_bind(
            self.id_B_stop,
            '<Button-1>',
            lambda e: [self.change_frame_flag("S_B"), self.stop(), self.changePage(self.stop_back_frame), self.start_blinking(self.id_S_B_stop)]
            )
        ''''''
    ...
    def start_blinking(self, img_id):
        ...
        if self.blinking_img_id is not None: # 既に点滅中の画像がある場合
            if self.flag == "F": 
                ...
                self.cvs_stop_back.itemconfigure(self.blinking_img_id, state='normal') #点滅してたシンボルを'表示'状態にして点滅停止
        '''
    ...
    def blink(self):
        ...
        if self.blink_state and self.blinking_img_id is not None:
            if self.flag == "F": # 前方走行時
            ...
            elif self.flag == "S_F": # 停止時＿前方
                current_state = self.cvs_stop_forward.itemcget(self.blinking_img_id, 'state') # 点滅させたいシンボルの状態を取得
                # 表示と非表示を繰り返して点滅させる
                if current_state == 'normal': 
                    new_state = 'hidden'
                else:
                ...
                self.cvs_stop_forward.itemconfigure(self.blinking_img_id, state = new_state) # シンボルの状態を変更
                self.blink_job = self.after(500, self.blink)# 繰り返し呼び出す

            elif self.flag == "S_B": # 停止時＿後方
                current_state = self.cvs_stop_back.itemcget(self.blinking_img_id, 'state') # 点滅させたいシンボルの状態を取得
                # 表示と非表示を繰り返して点滅させる
                if current_state == 'normal': 
                    new_state = 'hidden'
                else:
                ...
                self.cvs_stop_back.itemconfigure(self.blinking_img_id, state = new_state) # シンボルの状態を変更
                self.blink_job = self.after(500, self.blink)# 繰り返し呼び出す
        #'''
    '''画像の点滅を停止する関数'''
    def stop_blink(self):
        ...
        if not state_q.empty():
            ...
            if not state_msg:
                if self.flag == "F":
                    ...
                    self.change_frame_flag("S_F")
                    self.start_blinking(self.id_S_F_stop)
                elif self.flag == "B":
                    self.stop_blink()
                    self.changePage(self.stop_back_frame)
                    self.change_frame_flag("S_B")
                    self.start_blinking(self.id_S_B_stop)
                else:
                    pass
            state_q.task_done()
